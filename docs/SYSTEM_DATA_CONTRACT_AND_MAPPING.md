...
### 2.1 YBTY 原始数据源 (权威投注盘口与对阵基准)
- **地位**: **赛事标识与下注/结算盘口的唯一法定权威执行源**。
- **核心原则**:
  1. 所有进入推荐台账、展示给用户的队名必须 100% 保持 YBTY 原始字串；
  2. 所有下注方向、盘口线、赔率必须来自于 YBTY `markets`；
  3. 赛后盈亏结算、复盘命中率与滚球剩余进球/后续让球结算，全部以 YBTY 盘口为唯一法定依据。
...
### 2.2 雷速 (Leisu) 原始数据源 (基本面、技术统计、事件、初盘与即盘辅助预测)
- **地位**: **比赛基本面、伤停、阵容、事件流、技术统计及初盘/即盘辅助预测基准源**。
- **核心原则**:
  1. 雷速队名仅用于多维模糊匹配与交叉比对，绝不可替代 YBTY 队名；
  2. 雷速**初盘**（`opening`）与**即盘**（`instant` / `live`）用于衡量机构预期锚点与盘口衰减（Decay），辅助识别强队折价黄金期（Value Dilution）或诱深防爆冷预警；
  3. **严禁将雷速指数用作正式投注与结算赔率**。
