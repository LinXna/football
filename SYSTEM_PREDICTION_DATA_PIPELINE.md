# [DEPRECATED - 历史归档] CODEX 足球预测计算完整数据链路全景说明书

> [!CAUTION]
> **【历史架构归档声明 (ARCHIVED / DEPRECATED)】**  
> 本说明书制定于 2026-08-23，描述的是基于早期 Python 脚本采集匹配层 (`scripts/python/`) 及 `output/` 目录中转的历史架构。  
> **当前工程唯一事实来源 (SSOT)** 已全面升级为 **TypeScript 原生六层重构架构**，所有数据清洗、赛事对齐、量化博弈推演与台账审计均在 `/refactor/` 目录中原生执行：
> - 系统权威架构与数据契约：`/refactor/SYSTEM_ARCHITECTURE_AND_PIPELINE.md`
> - 核心量化模型与重构蓝图：`/refactor/SYSTEM_QUANT_REFACTOR_BLUEPRINT.md`
> - 任务进度与现行快照看板：`/refactor/HANDOVER_AND_PROGRESS.md`
> 
> 本文档仅作历史演进备忘参考，严禁作为新功能开发或算法改造的依据。

---

> **文档性质**：[历史归档] 系统核心架构全链路解析规范  
