...
> ⚠️ **历史诊断报告归档声明 (HISTORICAL DIAGNOSTIC ARTIFACT - REFERENCE ONLY)**
> 
> 本文档是 2026-09-01 阶段对 Layer 00 ~ 03 计算过程的历史诊断白皮书与排查快照。
> 文档中涉及的部分早期推演草案与探索性设想（例如将模型与庄家赔率分歧当做“逆向加码强化”或“分歧惩罚加权”等假设）已在系统后续演进（特别是在 Phase 5 专项治理与 Layer 05/06 落地）中被彻底废除与纠偏。
> **系统的唯一法定事实来源 (SSOT)** 为 `/refactor/SYSTEM_ARCHITECTURE_AND_PIPELINE.md` 与 `/refactor/DATA_SPECIFICATION.md`。任何算法与工程实现必须以最新的代码实现与规范文档为准，本文档仅保留作为历史计算切片与排查案例的参考资料。
