# Refactor Handover Archive (历史快照归档)

> 本文件为 `HANDOVER_AND_PROGRESS.md` 的历史快照归档（2026-09-07 ~ 2026-09-24）。
> 活动快照见 `HANDOVER_AND_PROGRESS.md`；冷启动入口见 `CONTINUATION.md`。

---

## 历史活动快照 (Historical Active Snapshots)

### SNAPSHOT-20260924-CODE-REVIEW-ISSUES-1-6 (DONE)
- **阶段进度**: `代码逻辑审查：发现 6 项问题，全部修复完成（问题 1~6）。`
- **审查发现**（按严重度）:
  1. **P1 settlement_basis 枚举断裂**：`candidateStateMachine` 只产生 `REST_OF_MATCH`/`FULL_MATCH`，`settlementEngine` 判断 `REMAINING_PERIOD_DOMINANCE`（死值，从不被产生），滚球让球/大小球「后续时段/剩余进球」结算分支永不触发，违反 AGENTS.md 规则 5/6；
  2. **P1 盘口 NaN 污染**：`parseAsianHandicapLine` 对无法解析字符串返回 `NaN`（vs `parseHandicapOrTotalLine`/`parseAsianLine` 返回 0），污染 `devigCalculator` EV 网格；
  3. **P1 三套盘口解析器并存**（`parseAsianHandicapLine`/`parseHandicapOrTotalLine`/`parseAsianLine`），行为不一致，违反 SSOT；
  4. **P2 parlayEngine LOSE_HALF 语义矛盾**（输半腿被赢腿抵消时仍误标 `LOSE_HALF`）；
  5. **P2 五态分布 `toFixed(4)` 后 ΣP ≠ 1.0**（注释声称 =1.0，逐项舍入累计偏差可达 ±0.00025）；
  6. **P3 getLeagueBaseGoals 别名覆盖不全**（「俄罗斯超级联赛」匹配不到「俄超」）。
- **本次修复**:
  1. 提取 `resolveSettlementBasis(market, stage)` helper，三处替换，按「赛前→FULL_MATCH / 滚球让球→REMAINING_PERIOD_DOMINANCE / 滚球大小球→REMAINING_GOALS」产生正确值，扩展 `PositiveEVSignal.settlement_basis` 类型；
  2. `devigCalculator` 的 `calculateAsianHandicapEV`/`calculateTotalGoalsEV` 加 NaN 防护（`Number.isFinite` 降级为 0）；
  3. `parlayEngine` 按有效赔率（而非「是否存在输半腿」）决定 `finalOutcome`：>1→WIN/WIN_HALF、=1→PUSH、0<x<1→LOSE_HALF/LOSE、=0→LOSE；
  4. `asianHandicap` 五态分布改用 `roundFiveStateToUnit` 最大余数法，保证 ΣP 精确 = 1.0；
  5. `poissonCore` 新增 `LEAGUE_ALIAS_MAP`（全称→简称 30+ 别名），`getLeagueBaseGoals` 先归一化全称再子串匹配；
  6. 三套盘口解析器 SSOT 统一：删除死代码 `parseHandicapOrTotalLine`（无调用点），`parseAsianLine` 改为委托 `parseAsianHandicapLine`（NaN→0 宽松封装），唯一事实来源 = `parseAsianHandicapLine`。
- **验证**: `tsc` 零错误 + `verify_settlement_engine` 46/46（新增 parseAsianLine 委托回归）+ `verify_p0_math_closure`（ΣP 精确闭合 1e-9 + 别名回归）+ `verify_candidate_state_machine`/`verify_layer04_05_candidate_boundary`/`verify_full_pipeline_00_03`/`verify_quant_engine` 全绿。
- **遗留待办**: 无（6 项全部完成）。


### SNAPSHOT-20260924-HALF-TIME-EV-COMPLETION (DONE)
- **阶段进度**: `待办1：半场 EV 贯通全部完成（P2 级功能补全，非缺陷）。`
- **交付成果**:
  1. `index.ts` 信号接入：`calculateConfidenceAndAlerts` 新增半场让球（`half_spread_main_ev` → `ASIAN_HANDICAP_HALF`）与半场大小球（`half_total_main_ev` → `TOTAL_GOALS_HALF`）信号提取；
  2. `index.ts` `toOosMarket` 新增 `ASIAN_HANDICAP_HALF`/`TOTAL_GOALS_HALF` 映射（避免被候选状态机 UNSUPPORTED_MARKET 阻断）；
  3. `promptExporter.ts` `core_markets` 新增 `ah_half`（`annotateAhMarket(half_spread_main, half_spread_main_ev)`）与 `ou_half`（`annotateOuMarket(half_total_main, half_total_main_ev)`）导出；
  4. 新建 `verify_half_time_quant_engine.ts` 三层验证（底层 EV 求解 / 信号接入 / 端到端 + prompt 导出）。
- **验证结果**: `npx tsc --noEmit` 零错误 + `verify_half_time_quant_engine.ts` 全绿 + 回归 `verify_quant_engine`/`verify_p0_math_closure`/`verify_candidate_state_machine`/`verify_ai_evaluator` 全绿。


### SNAPSHOT-20260924-P3-3-TESTS-OLD-SYSTEM-DECOUPLING (DONE)
- **阶段进度**: `待办2：tests/ 下 3 个脚本引用旧系统 server 的解耦全部完成。`
- **根因**: 旧系统 `server/services/oosArchiveService.ts` 实为「寄生在 refactor 上的薄壳」（import refactor 的 `oosCalibrationEngine.js` + `types.js`，只做文件 IO + 缓存 + 状态查询）；refactor 缺 OOS 持久化服务层，3 个测试脚本被迫引旧系统。
- **交付成果**:
  1. 新建 `06_settlement_audit/oosArchiveService.ts`：等价迁移 OOS 持久化服务层（`ensureOosArchiveInitialized`/`getLoadedOosArchive`/`getOosStatus`/`appendSampleAndRebuildArchive`），只依赖 refactor 内部，遵循原子写范式 + 强类型零 any；
  2. 迁移 `profile_steps.ts`：`getLoadedOosArchive` 改从 refactor 内部 import；
  3. 迁移 `verify_p3_ledger_snowball.ts`：`appendSampleAndRebuildArchive`/`getOosStatus` 改从 refactor 内部 import；
  4. 删除 `benchmark_import.ts`（旧系统 `canonicalRoutes.assembleMatchesForMode` 性能基准，非 refactor 体系，放错位置）。
- **验证结果**: `npx tsc --noEmit` 零错误 + `verify_p3_ledger_snowball.ts` 全绿（Task 3.3 雪球递增验证通过，功能等价）+ refactor 目录内旧系统 import 归零。
- **遗留提示**: 旧系统 `server/services/oosArchiveService.ts` 仍反向依赖 refactor（旧系统侧问题，留待旧系统迁移时处理，不在 refactor 操作范围）。


### SNAPSHOT-20260924-P3-DOC-GOVERNANCE (DONE)
- **阶段进度**: `原子任务 G（P3 全项）完成：文档治理。`
- **交付成果**:
  1. P3-1 交接文档瘦身：`HANDOVER_AND_PROGRESS.md` 200KB→0.5KB（2283 行→13 行），全部历史快照归档至 `archive/HANDOVER_ARCHIVE.md`；
  2. P3-2 废弃文档清理：`implementation_plan.md`（已标注废弃）移入 `archive/`；
  3. P3-3 旧系统耦合确认：生产代码（00-06 层）零 import 旧系统（git grep 验证）；tests/ 下 `benchmark_import`/`profile_steps`/`verify_p3_ledger_snowball` 3 处仍引用旧系统 server（跨系统对比/集成验证用途，已记录待后续评估）；
  4. 文档同步：`CONTINUATION.md` 历史引用指向 `archive/HANDOVER_ARCHIVE.md`。
- **改动文件清单**: `archive/HANDOVER_ARCHIVE.md`（新建，历史快照归档）、`archive/implementation_plan.md`（移入）、`HANDOVER_AND_PROGRESS.md`（瘦身）、`CONTINUATION.md`、`REFACTOR_PRIORITY_REPORT.md`。

### SNAPSHOT-20260924-P1-1-SPLIT-GOD-FILES (DONE)
- **阶段进度**: `拆分 03 层上帝文件（P1-1）全部完成。4 个上帝文件（contextEngine/poissonDecayModel/momentumQuantEngine/devigCalculator）全部物理拆分。`
- **交付成果**:
  1. contextEngine.ts（1787→153）拆成 6 子模块 + 聚合入口桶；
  2. poissonDecayModel.ts（1015→702）拆出 poissonCore/poissonDecay；
  3. momentumQuantEngine.ts（1016→672）拆出 momentumMath/momentumTimeline；
  4. devigCalculator.ts（1129→758）拆出 devigMath/asianHandicap，保留 EV 计算 + 聚合入口；
  5. 跨层引用保持稳定：parseAsianHandicapLine/invertHandicapString/calculateDeviggedMarketFeatures 仍从 devigCalculator.js 供 alignmentGuard.ts 与 index.ts 直接 import；
  6. 验证全绿：tsc 零错误 + verify_quant_engine / verify_regression_fixes / verify_p0_math_closure / verify_full_pipeline_00_03 / verify_p2_tactical_momentum / verify_ai_evaluator。
- **改动文件清单**: 12 个新建子模块（l0CircuitBreaker/h2hDecay/recentForm/lineupImpact/goalDistribution/motivationUrgency/poissonCore/poissonDecay/momentumMath/momentumTimeline/devigMath/asianHandicap）+ 4 个精简桶（contextEngine/poissonDecayModel/momentumQuantEngine/devigCalculator）。

### SNAPSHOT-20260924-P1-2-P1-3-DOCS-LAYER-BALANCE (DONE)
- **阶段进度**: `原子任务 E 完成：P1-2 文档移出源码 + P1-3 05/06 层粒度再平衡评估。`
- **交付成果**:
  1. P1-2：`EXPERT_DIAGNOSIS.md`（6420 行/206KB）从 `03_quant_engine/` 移出至 `refactor/` 根目录（git mv 保留历史），核查确认未被任何 `.ts` 引用；
  2. P1-3：评估 06 层 8 文件 + 05 层 4 文件，结论「维持现状、不合并」——formalLedgerAdapter（批量转换）与 ledgerRecordAdapter（单条字段后补）职责不同，parlayEngine/settlementEngine 职责分离合理，小文件清晰边界符合单一职责原则；
  3. 验证：`npx tsc --noEmit` 零错误 + `git diff --check` 通过。
- **改动文件清单**: `/refactor/03_quant_engine/EXPERT_DIAGNOSIS.md` → `/refactor/EXPERT_DIAGNOSIS.md`（重命名）、`/refactor/REFACTOR_PRIORITY_REPORT.md`、`/refactor/HANDOVER_AND_PROGRESS.md`。

### SNAPSHOT-20260924-P2-HYGIENE-CLEANUP (DONE)
- **阶段进度**: `原子任务 F（P2 全项）完成：目录卫生清理。`
- **交付成果**:
  1. P2-1：删除 2 个 `.corrupt-*` 损坏快照（各 549KB），核查确认无代码引用；
  2. P2-2：删除 `guam_pipeline_inspection_result.json` 一次性文件；5 个 README 按层组织（01/02/03）属合理分层，保留；
  3. P2-3：评估结论「维持现状」——447 处 console.log 全在 tests/（生产代码已清零），统一断言框架属长期改进，不在本次目录清理范围；
  4. P2-4：globalTierMatrix 配置抽离——231 行配置数据（FootballTier/TierProfile/TIER_PROFILES/NATIONAL_TEAM_TIERS/CLUB_TIERS/LEAGUE_TIERS）抽至 `tierData.ts`，`globalTierMatrix.ts` 精简为 105 行纯逻辑 + re-export，4 个消费方（contextEngine/h2hDecay/prematchPriorEngine/recentForm）API 稳定；
  5. 验证：`npx tsc --noEmit` 零错误 + verify_quant_engine（11/11）+ verify_full_pipeline_00_03（100%）。
- **改动文件清单**: 新建 `/refactor/03_quant_engine/tierData.ts`、精简 `/refactor/03_quant_engine/globalTierMatrix.ts`、删除 2 个 `.corrupt-*` + `/refactor/samples/guam_pipeline_inspection_result.json`、`/refactor/REFACTOR_PRIORITY_REPORT.md`、`/refactor/HANDOVER_AND_PROGRESS.md`。

### SNAPSHOT-20260924-P0-2-REMAINING-ANY-CLEANUP (DONE)
- **阶段进度**: `生产代码 any 全面清零（41 → 0），剩余 2 处均为注释文字。`
- **交付成果**:
  1. 补字段根治类型契约断层（competition、reason/injury_reason、squad_market_value、model_probability/probability/minute、team_side、current、attack_momentum_timeline、hard_gate_ceiling 补 C_GRADE）；
  2. 消除冗余兜底（reference.live/pregame/initial、full_spread_main.line、calcAvgAge 字符串兜底）；
  3. 类型安全收窄（catch、checkQuarterLine、annotate、isQuarterOrSplitLine 用 unknown 方案）；
  4. 验证全绿：tsc 零错误 + 6 个 verify 测试全通过。
- **改动文件清单**: `/refactor/01_data_ingestion/leisu/types.ts`、`/refactor/02_canonical_model/types.ts`、`/refactor/02_canonical_model/dataConsistencyAuditor.ts`、`/refactor/03_quant_engine/contextEngine.ts`、`/refactor/03_quant_engine/marketDivergenceEngine.ts`、`/refactor/05_portfolio_risk/ledgerPersistence.ts`、`/refactor/04_ai_evaluator/promptExporter.ts`、`/refactor/04_ai_evaluator/types.ts`。

### SNAPSHOT-20260924-P0-1-ALIGNMENT-GUARD-ANY-ROOT-CAUSE (DONE)
- **阶段进度**: `根治 alignmentGuard.ts 的 15 处 any 类型断层完成，生产代码 any 41 → 19。`
- **交付成果**:
  1. alignmentGuard.ts 15 处 any 全部根治（risk_adjusted_ev 断层 + direction + isQuarterOrSplitLine + checkAhMatch/checkOuMatch/euro_1x2 + findFirstLegallyVerifiableSignal 签名）；
  2. 03 层 PositiveEVSignal 补 risk_adjusted_ev?: number；04 层新增 CoreMarketLine/CoreEuroMarket 类型替代 core_markets 的 7 个 any；
  3. 验证全绿：tsc 零错误 + verify_ai_evaluator / verify_layer04_05_candidate_boundary / verify_traceability_matrix / verify_quant_engine 100% 通过。
- **改动文件清单**: `/refactor/03_quant_engine/types.ts`、`/refactor/04_ai_evaluator/types.ts`、`/refactor/04_ai_evaluator/alignmentGuard.ts`。

### SNAPSHOT-20260922-HALF-TIME-INDEPENDENT-QUANT-EVALUATION (STALLED - 未完成，搁置待办)
- **阶段进度**: `上下半场独立定价信号（半场 EV）贯通链条，2026-09-24 复核为半成品：底层已算好、上层三件事未接入。`
- **实际完成度核验**:
  1. ✅ 底层半场 EV 计算已存在：`03_quant_engine/devigCalculator.ts` 的 `calculateDeviggedMarketFeatures` 已输出 `half_spread_main_ev` / `half_total_main_ev`（行 1115-1116）；
  2. ✅ 类型契约已定义：`03_quant_engine/types.ts` 的 `DeviggedMarketFeatures` 已含 `half_spread_main_ev?` / `half_total_main_ev?`（行 630-631），`OosMarket` 已含 `ASIAN_HANDICAP_HALF` / `TOTAL_GOALS_HALF`；
  3. ❌ 上层信号接入未做：`03_quant_engine/index.ts` 的 `calculateConfidenceAndAlerts` 尚未接入 `half_*_ev` 生成 positive_ev_signals；
  4. ❌ 提示词导出未做：`04_ai_evaluator/promptExporter.ts` 尚未序列化 `ah_half` / `ou_half`；
  5. ❌ 回归测试未做：`tests/verify_half_time_quant_engine.ts` 不存在。
- **遗留待办（Resume 续做）**: 完成上述 3~5 三项，闭环半场信号链条。
...
### Snapshot: SNAPSHOT-20260918-DIRECTNESS-AND-SHOT-QUALITY-METRICS (DONE)
- **阶段进度**: `P5.9.1 足球量化系统深度重构工程 —— 反击纵向锐度比 (Directness Ratio) 与射正扑救成色代理 (Shot Quality & Save Severity) 落地实施 [已全面完成并通过全量验证]`
- **交付产物**:
  - 在 `types.ts` 与 `momentumQuantEngine.ts` 落地反击纵向锐度比 $D_{\text{ratio}}$；
  - 在 `eventMomentumFusion.ts` 落地射正扑救成色代理与文字直播重大险情识别；
  - 全套 115 项测试与端到端集成 100% 绿灯通过。
