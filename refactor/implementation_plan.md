...
- 当 `divergence_delta`（理论与市场的偏差）极大时，进一步压低市场权重，暗示市场可能在"做盘"
...
> [!IMPORTANT]
> 这是一个**架构级修改**。修改后所有使用 `calibrateWithMarketOdds()` 输出的下游模块（M4 Poisson、M5 Devig）的数值都会发生变化。必须确保现有 9 个回归测试全部通过。

## Proposed Changes

### Layer 03 量化引擎

#### [MODIFY] [marketDivergenceEngine.ts](file:///D:/开发/football/CODEX/refactor/03_quant_engine/marketDivergenceEngine.ts)
...
#### [MODIFY] [types.ts](file:///D:/开发/football/CODEX/refactor/03_quant_engine/types.ts)
...
#### [MODIFY] [verify_quant_engine.ts](file:///D:/开发/football/CODEX/refactor/tests/verify_quant_engine.ts)
