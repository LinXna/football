...
- **联合反演与混合贝叶斯收缩**：
  ...
  - **Physics-First 贝叶斯先验融合**：删除了固定 85%/15% 权重；赛前基础权重为市场 60%/理论 40%，滚球市场权重按分钟从 0.55 递减并受最低值约束，理论与市场偏差还会额外扣减市场权重，最终权重写入 `MarketCalibrationResult` 与 `lambda_decomposition` 审计输出。
