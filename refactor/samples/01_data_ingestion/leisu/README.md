...
| :--- | :--- | :--- | :--- | :--- |
| **DS-01** | **实时赛事元数据与比分流**<br>`(Realtime Match & Score Stream)` | 比赛 ID、主客队 ID/队名、赛事 ID/赛事名、开赛时间、比赛阶段状态 (`status_id`)、全场/半场比分、比分可靠性核验标记 (`score_verified`) | [第 1、2 节](#1-顶层元数据结构-root-level) | Layer 01 / 02 实体对齐与比分核验 |
...
> ⚠️ **重要时间体系边界说明**：  
> 1. **雷速接口本身不提供比赛即时滚球进行时钟**：雷速原始接口只提供比赛生命周期状态代码 `status_id`、中文描述 `status_text` 以及文字直播各事件发生的时间戳 `timeline_events[i].minute`。  
> 2. **严禁将雷速 `text_live` 当做比赛正在进行的时间标签**：`text_live` 是离散的事件发生时间（Event Incident Timestamp），无事件时时间停滞，绝非连续进行的时钟！  
> 3. **滚球即时比赛分钟数 (`minute`) 唯一法定事实来源 (SSOT)**：必须且只能从交易盘口发生地 YBTY 的即时时钟 (`ybty.clock`) 解析获得（如 `"62:25"` $\rightarrow$ `62`）。  
> 4. **雷速端时间轴印证唯一途径**：若需在雷速端核验比赛时间轴进度，**只能使用动量走势点阵长度 (`attack_momentum` 点阵长度)**（上半场点数 + 下半场点数 = 已进行分钟数）。
