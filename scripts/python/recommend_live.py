...
def assess(candidate: dict[str, Any], now: datetime) -> dict[str, Any]:
    ...
    live_efficiency = calculate_live_efficiency(stats, score)
    text_entries = [
        str(item)
    ...
    for side in ("home", "away"):
        efficiency = live_efficiency["teams"][side]
        evidence.append(
            f"{side}射门效率：进球/记录射门={efficiency['goal_conversion_per_recorded_shot']}，"
            f"进球/射正={efficiency['goal_conversion_per_shot_on_target']}，射正率={efficiency['shot_accuracy']}"
        )
    risks.extend(live_efficiency["warnings"])
        ...
        existing.append(
            {
                ...
                "ybty_league": decision.get("ybty_league"),
                "leisu_league": decision.get("leisu_league"),
                ...
                "data_context": {
                    ...
                    "interface_features": decision.get("interface_features", {}),
                    "live_efficiency": decision.get("live_efficiency", {}),
                },
                "prediction_features": {
